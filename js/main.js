...
$(document).ready(setResizeContactForm);

function setResizeContactForm() {
  resizeContactForm();
  window.addEventListener("resize", resizeContactForm);
}

function resizeContactForm() {
  var width = window.innerWidth;
  var frame = document.querySelector("#contact .popup-inner iframe");
  if (width <= 375) {
    frame.width = 300;
  ...
  $(".navbar-toggle").on("click", function() {
    ...
    if ($("#burger").hasClass("active")) {
      ...
      $(".navbar-default .navbar-brand").css(
        "color",
        returnColour(pagePositionScrolled())
      );
      $(".navbar-toggle .icon-bar:nth-of-type(1)").css(
        "background-color",
        returnColour(pagePositionScrolled())
      );
      $(".navbar-toggle .icon-bar:nth-of-type(3)").css(
        "background-color",
        returnColour(pagePositionScrolled())
      );
...
$(document).ready(function() {
  $(document).on("click", function(event) {
    ...
    if (
      $(".navbar-collapse").hasClass("collapse in") &&
...
function toggleBurger() {
  ...
  if ($("#burger").hasClass("active")) {
    $(".navbar-toggle.active .icon-bar:nth-of-type(1)").css({
      top: "6px",
      transform: "rotate(45deg)"
    });
    $(".navbar-toggle.active .icon-bar:nth-of-type(3)").css({
      top: "-6px",
      transform: "rotate(-45deg)"
    });
    $(".navbar-toggle.active .icon-bar:nth-of-type(2)").css({
    ...
    if (i.pageX < $(window).width() * 0.25) {
      ...
      if (!cue) {
        ...
        if (difX <= 0 && difY <= 0) {
          posCount += 1;
          if (y < 1 && posCount > 10) {
            $("#feedback h2 strong").text(
              "Before you leave... leave some feedback"
            );
            openFeedback();
          }
        } else {
...
function popupWhenBottom() {
  ...
  if (
    bottomReached &&
    $(this).scrollTop() + $(this).height() < bottom - 140 &&
    !$("#contact").hasClass("show-popup")
  ) {
    openFeedback();
  }
...
function closeFeedback() {
  ...
  $("#feedback .btn-feedback").on("click", function(event) {
    $("#feedback .feedback-contents").slideUp(600, function() {
      $(this).remove();
    });
    $("#feedback .popup-inner").append(
      '<div class="row text-center" style="margin-top: 2em;"><h2><strong>Thank you for the feedback!</strong></h2></div>'
    );
  ...
  $("#feedback .btn-good").on("click", function(event) {
    ...
    $("#feedback .feedback-contents").slideUp(speed, function() {
      $(this).remove();
    });
    $("#feedback .popup-inner").append(
      '<div class="row text-center" style="margin-top: 2em;"><h2><strong>Thank you for the feedback!</strong></h2></div>'
...
function makeContactFormScrollable() {
  $("body").css("overflow-y", "hidden");
  document.getElementById("contact").style["overflow-y"] = "scroll";
  // $("#contact").css("overflow-y", "scroll");
  $("#contact-wrapper").switchClass(
...
function scrollContact() {
  $("#contact .popup-main iframe").hover(makeContactFormScrollable);
  $("#contact .popup-main iframe").on("click", makeContactFormScrollable);
}
